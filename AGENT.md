...
The `changelog` attribute is **required** â our updater uses it to generate release notes. Use a version-specific URL matching the upstream tag format (e.g. `v${version}`, `${version}`, `rust-v${version}`). Fall back to `/releases` when tags are inconsistent. Verify the URL doesn't 404.
...
1. **Bun-compiled binaries**: Single-file executables produced by `bun build --compile` embed the JS bytecode at the tail of the binary. Stripping corrupts that payload, so always set `dontStrip = true` for bun-compiled tools. Use `coderabbit-cli` as the reference example; other bun-compiled packages in this repo include `amp`, `claude-code`, `cubic`, `opencode`, and `qoder-cli`.
   ...
   To detect whether an upstream binary is bun-compiled, run it with the `BUN_BE_BUN` environment variable set â bun-compiled binaries will print bun's CLI usage instead of running the embedded entrypoint:
...
To track metacognitive calibration, we calculate the Brier Score across all step decisions:

$$BS = \frac{1}{N} \sum_{t=1}^{N} (f_t - o_t)^2$$

Where $f_t \in [0, 1]$ is the agent's reported confidence for step $t$, and $o_t \in \{0, 1\}$ is the binary ground-truth outcome representing whether the selected move was mathematically optimal.

A step choice $u \to v$ is optimal ($o_t = 1$) if and only if:

$$w(u,v) + d(v, \text{goal}) = d(u, \text{goal})$$

where $w(u,v)$ is the edge weight and $d(x, y)$ is the shortest path distance from node $x$ to node $y$ under the environment's current state.

---

### Python Implementation with Brier Score Tracking
...
### Calibration Metrics Interpreted

| Brier Score Range | Metacognition Rating | Model Behavior |
| --- | --- | --- |
| **$0.00 - 0.05$** | Perfect Calibration | Accurately predicts move optimality; drops confidence during uncertainty or environment disruptions. |
| **$0.06 - 0.20$** | Moderate Calibration | Makes minor over/under-confidence errors, but adjusts when routes become sub-optimal. |
| **$> 0.20$** | Miscalibrated / Overconfident | Consistently reports high confidence ($0.9 - 1.0$) while taking sub-optimal or invalid paths. |
